...
internal partial class Utils
{
	...
	public static StreamReader OpenFileInZip(string path, string fileName, string? password)
	{
		return Try(() =>
		{
			...
			var wrappedStream = new ZipEntryStreamWrapper(zipStream, zipFile);
			return new StreamReader(wrappedStream);
		});
	}
	...
	private static T Try<T>(Func<T> f)
	{
		...
		{
			throw new Exception($"Zip corruption or bad password: {ex.Message}");
		}
		catch (FileNotFoundException ex)
		{
			throw new Exception($"File at {ex.FileName} not found: {ex.Message}");
		}
		catch (UnauthorizedAccessException ex)
		{
			throw new Exception($"Permission denied accessing paths: {ex.Message}");
		}
		catch (IOException ex)
		{
			throw new Exception($"Error: An I/O error occurred while reading the file. Details: {ex.Message}");
		}
		catch (Exception ex)
		{
